...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        background-color: \
                           ...
                           font: bold 20px; \
                           padding: 3px; min-width: 1em; border-style: outset;")
        self.score_info = QTextBrowser(self.centralwidget)
        self.score_info.setGeometry(QtCore.QRect(20, 540, 140, 50))
        self.score_info.setObjectName("textBrowser")
        self.score_info.setStyleSheet("padding: 10px;\
                background-color: \
                                   rgb(204, 204, 255); \
        ...
        self.Results_of_test_more_info.setGeometry(QtCore.QRect(70, 80, 360, 480))
        ...
        self.score_info.setHidden(True)
...
class MyWidget(QMainWindow, Ui_MainWindow):
    ...
    def start_test(self):
        ...
        self.score_info.setText('1 Балл')
        self.score_info.setHidden(False)
    ...
    def next_question(self):
        ...
        if (text_answer == '' or len([x for x in text_answer if x not in '1234567890,-']) != 0) and not self.skip_bool:
            ...
            if self.question_number >= 10:
                ...
                if 7 <= self.question_number <= 9:
                    self.score_info.setText('2 Балла')
                else:
                    self.score_info.setText('1 Балл')
                id_select = random.randint(1, 10)
                self.list_of_id.append(id_select)
    ...
    def results_of_test(self):
        ...
        procents_of_comparison = int((((self.score / 13) - (int(self.past_score) / 13)) * 100))
        if procents_of_comparison > 0:
            comparing_text = f'      Вы набрали на {procents_of_comparison}% больше баллов, чем раньше!'
        elif procents_of_comparison < 0:
            comparing_text = f'      Вы набрали  на {-1 * procents_of_comparison}% меньше баллов, чем раньше!'
        else:
